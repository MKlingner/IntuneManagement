...
function Import-GraphFile
{
    ...
    {
        Write-Log "chkImportAssignments: true"
    ...
    try
    {
        ...
        Write-Log "Assignments: $($newObj | ConvertTo-Json -Depth 50 -compress) && $($objClone.Assignments | ConvertTo-Json -Depth 50 -compress)"
...
function Reset-GraphObject
{
    ...
    elseif(($curObject | measure).Count -eq 1)
    {
        ...
        Remove-Property $obj "isMigratingToConfigurationPolicy"
        ...
        if($global:cbImportType.SelectedValue -eq "update")
        {
            ...
            $json = ConvertTo-Json $obj -Depth 50 -Compress
            ...
            {
                Import-GraphObjectAssignment $curObject.Object $objectType $assignments $fileObj.FileInfo.FullName | Out-Null
            }
            return $true
        }
        elseif($global:cbImportType.SelectedValue -eq "replace")
        {
            ...
            Write-Log "PostReplace: $($newObj | ConvertTo-Json -Depth 50 -compress) && $($replace)"
            ...
            if($newObj -and $replace)
            {
                ...
                Write-Log "Assignments: $($curObject.Object | ConvertTo-Json -Depth 2 -compress)"
...
function Import-GraphObjectAssignment
{
    ...
    Write-Log "Import assignments for $($objectType.Title) object $(($assignments | Measure-Object).Count) assignments found"
    Write-Log "Assignments: $(@{Assignments=$assignments} | ConvertTo-Json -Depth 3 -compress)"
    ...
    {
        Write-Log "Import Assignment: $($assignment | ConvertTo-Json -Depth 3 -compress)"
        ...
        # Ensure in case that Id is no Property of the assignment
        $assignment | Add-Member -MemberType NoteProperty -Name Id -Value "" -Force
    ...
    $json = $htAssignments | ConvertTo-Json -Depth 50 -Compress
    if($CopyAssignments -ne $true)
    {
        Write-Log "Before Update-JsonForEnvironment: $json"
        $json = Update-JsonForEnvironment $json
        Write-Log "After Update-JsonForEnvironment: $json"
    }
...
"@
                ...
                if($obj)
                {
                    Write-Log "Mapping Group '$($migObj.DisplayName)' $($migObj.Id) -> $($obj.Id)"
